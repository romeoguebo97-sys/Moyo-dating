...
function Landing({ onNav }: { onNav: (p: string) => void }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: G.creme, overflow: "hidden" }}>
      ...
      {/* ── NAV ── */}
      <nav style={{ background: G.blanc, boxShadow: "0 2px 16px rgba(44,26,14,0.07)", position: "sticky", top: 57, zIndex: 100 }}>
...
function Messages({ auth, onUnreadCount, onShowPremium }: { auth: Auth; onUnreadCount: (n: number) => void; onShowPremium: (r: string) => void }) {
  ...
  if (open) return <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 130px)" }}><div style={{ padding: "12px 16px", background: G.blanc, borderBottom: `1px solid ${G.gris}`, display: "flex", gap: 12, alignItems: "center", position: "sticky", top: 0, zIndex: 10 }}><div onClick={() => { setOpen(null); loadConvs(); }} style={{ cursor: "pointer", fontSize: "1.1rem", color: G.brunLight, padding: "4px 8px" }}>←</div><Avatar url={open.partner?.photo_url} gender={open.partner?.gender} size={38} premium={open.partner?.is_premium} /><div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: "0.92rem" }}>{open.partner?.name}</div><div style={{ fontSize: "0.7rem", color: "#27ae60" }}>● Actif</div></div>{!auth.isPremium && <div style={{ fontSize: "0.7rem", color: G.brunLight, background: G.creme, padding: "4px 8px", borderRadius: 50 }}>{Math.max(0, FREE_LIMITS.messages - msgCount)}/{FREE_LIMITS.messages} msg</div>}<div onClick={() => setShowDeleteConv(true)} style={{ cursor: "pointer", padding: "6px 8px", borderRadius: 8, color: "#e74c3c", fontSize: "1rem", opacity: 0.7 }}>🗑️</div></div><div style={{ flex: 1, overflowY: "auto", padding: "14px", display: "flex", flexDirection: "column", gap: 10 }}>{msgs.length === 0 && <div style={{ textAlign: "center", color: G.brunLight, padding: "24px 0", fontSize: "0.85rem" }}>Dites bonjour ! 👋</div>}{msgs.map((m, i) => <div key={i} style={{ display: "flex", justifyContent: m.sender_id === auth.userId ? "flex-end" : "flex-start" }}><div style={{ background: m.sender_id === auth.userId ? G.rouge : G.blanc, color: m.sender_id === auth.userId ? G.blanc : G.brun, padding: "10px 14px", borderRadius: m.sender_id === auth.userId ? "18px 18px 4px 18px" : "18px 18px 18px 4px", maxWidth: "72%", fontSize: "0.88rem", lineHeight: 1.5 }}>{m.content}</div></div>)}<div ref={bottomRef} /></div><div style={{ padding: "10px 12px", background: G.blanc, borderTop: `1px solid ${G.gris}`, display: "flex", gap: 8 }}><input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Écris un message..." style={{ flex: 1, padding: "11px 14px", border: `2px solid ${G.gris}`, borderRadius: 50, fontSize: "0.88rem", outline: "none", background: G.creme }} /><div onClick={send} style={{ width: 40, height: 40, borderRadius: "50%", background: G.rouge, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: "0.95rem", color: G.blanc, flexShrink: 0 }}>➤</div>{showDeleteConv && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}><div style={{ background: G.blanc, borderRadius: 20, padding: "28px 24px", width: "100%", maxWidth: 320, textAlign: "center", boxShadow: "0 20px 60px rgba(44,26,14,0.2)" }}><div style={{ fontSize: "2rem", marginBottom: 12 }}>🗑️</div><h3 style={{ fontFamily: "Georgia,serif", fontSize: "1.1rem", fontWeight: 700, marginBottom: 8, color: G.brun }}>Supprimer la conversation ?</h3><p style={{ fontSize: "0.82rem", color: G.brunLight, marginBottom: 20, lineHeight: 1.5 }}>Tous les messages seront supprimés. Cette action est irréversible.</p><div style={{ display: "flex", gap: 10 }}><Btn variant="ghost" onClick={() => setShowDeleteConv(false)} style={{ flex: 1 }}>Annuler</Btn><Btn variant="danger" onClick={deleteConv} style={{ flex: 1 }}>Supprimer</Btn></div></div></div>}</div></div>;
