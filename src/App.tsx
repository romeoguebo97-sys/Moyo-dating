...
function Messages({ auth, onUnreadCount, onShowPremium }: { auth: Auth; onUnreadCount: (n: number) => void; onShowPremium: (r: string) => void }) {
  ...
  const deleteConv = async () => { if (!open) return; if (!window.confirm("Supprimer cette conversation ?")) return; await sb.delete(auth.token, "messages", `?match_id=eq.${open.id}`); setOpen(null); loadConvs(); };
