...
function SignUp({ onNav }: { onNav: (p: string) => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Se connecter immédiatement pour avoir un vrai token
      const loginRes = await sb.signIn(emailClean, form.password);
      const realToken = loginRes?.access_token;
      const userId = loginRes?.user?.id || authRes.user?.id;

      // Upload photo avec le vrai token de session
      if (photoFile && userId && realToken) {
        try {
          // Attendre que le profil soit bien créé
